// Package upgrade replaces the running mcpify binary with the latest GitHub
...
const releaseAPI = "https://api.github.com/repos/aloki-alok/mcpify/releases/latest"
...
func Run(ctx context.Context, current string) (tag string, updated bool, err error) {
	...
	archive := fmt.Sprintf("mcpify_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
...
func get(ctx context.Context, url, accept string) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", "mcpify")
	if accept != "" {
...
func extractBinary(tarball []byte) ([]byte, error) {
	...
	for {
		...
		if path.Base(hdr.Name) == "mcpify" {
			return io.ReadAll(tr)
		}
	}
	return nil, fmt.Errorf("mcpify binary not found in release archive")
}
